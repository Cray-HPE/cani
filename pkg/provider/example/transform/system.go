...
func TransformSystem(existing devicetypes.Inventory, data *import_.SystemCSV) (*devicetypes.TransformResult, error) {
	...
	if err = transformSystemInterfaces(data, result); err != nil {
...
// and Name as the interface name. Rows referencing an unknown owner or
// interface are logged and skipped so a single bad row does not abort import.
func transformSystemInterfaces(data *import_.SystemCSV, result *devicetypes.TransformResult) error {
