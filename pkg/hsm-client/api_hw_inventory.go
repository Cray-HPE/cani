...
type HWInventoryApiService service

/*
HWInventoryApiService Retrieve results of HWInventory query starting at {xname}
 ...
 * @param xname Locational xname of parent component, system (e.g. s0, all) or partition (p#.#) to target for hardware inventory
 * @param optional nil or *HWInventoryApiDoHWInvByLocationQueryGetOpts - Optional Parameters:
     ...
     * @param "Children" (optional.Bool) -  Also return children of the selected components. Default is true.
     * @param "Parents" (optional.Bool) -  Also return parents of the selected components.
     * @param "Partition" (optional.String) -  Restrict search to the given partition (p#.#). Child components are assumed to be in the same partition as the parent component when performing this kind of query.
     * @param "Format" (optional.String) -  How to display results   FullyFlat      All component types listed in their own                  arrays only.  No nesting of any children.   NestNodesOnly  Flat except that node subcomponents are nested                  hierarchically. Default is NestNodesOnly.
@return HwInventory100HwInventory
*/

type HWInventoryApiDoHWInvByLocationQueryGetOpts struct {
...
func (a *HWInventoryApiService) DoHWInvByLocationQueryGet(ctx context.Context, xname string, localVarOptionals *HWInventoryApiDoHWInvByLocationQueryGetOpts) (HwInventory100HwInventory, *http.Response, error) {
	var (
		localVarHttpMethod  = strings.ToUpper("Get")
		localVarPostBody    interface{}
		localVarFileName    string
		localVarFileBytes   []byte
		localVarReturnValue HwInventory100HwInventory
	)

	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/Inventory/Hardware/Query/{xname}"
	...
	if localVarHttpResponse.StatusCode >= 300 {
		...
		if localVarHttpResponse.StatusCode == 200 {
			var v HwInventory100HwInventory
